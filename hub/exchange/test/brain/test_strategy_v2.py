...
class V2StrategyTest(TestCase):
    ...
    def setUp(self):
        ...
        self.testEvents= [  
                            {'source':str(sensorId), 'attribute_name':'Bar', 
                            'parameter_name':'Foo','value':'5', 
                            'request_id':None}, 

                            {'source':str(sensorId), 'attribute_name':'FuBar', 
                            'parameter_name':'Foo','value':'10', 
                            'request_id':None},

                            {'source':str(userActionId), 'attribute_name':'Foo', 
                            'parameter_name':'Foo','value':'5', 
                            'request_id':2}
                         ]
        ...
        self.testNonTriggeringEvent.data = {"event" : "device.event",  
                                     "attribute" : { "name" : "Bar", 
                                                    "parameters": [{
                                                        "name" : "Foo",
                                                        "value" : "10"
        ...
        self.testTriggeringEvent.data = {"event" : "device.event",  
